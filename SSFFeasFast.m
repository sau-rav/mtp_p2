% BCD + Fast Gradient method for solving the feasibility problem of 
...
%
% For solving the convex subproblem Fast Gradient method has been used
%
% Input : A, B, options
...
%          t          : timestamps
% ----------------------------------------------------------------------
function [S, L, K_final, flag, e, t] = SSFFeasFast(A, B, options)
    ...
    fprintf('Solving BCD for checking feasibility...\n');
    while i < options.itermax ...
        && (i <= 5 || e(i - 1) - e(i - 2) > 1e-6)
        
        if options.display == 2
            fprintf('iteration : %d\n', i); 
        end
        % keep S as constant solve for L
        Ln = fastGradL(A, B, B_inv, T, L);
        if options.display == 2
